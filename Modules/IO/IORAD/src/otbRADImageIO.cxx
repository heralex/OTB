...
bool RADImageIO::InternalReadHeaderInformation(const std::string& file_name, std::fstream& file, const bool reportError)
{
  ...
  else
  {
    if (reportError == true)
    {
      itkExceptionMacro(<< "RAD : the value type '" << lStrCodePix << "' (second line) set in the header file is not recognized as correct value.");
    }
    else
    {
      return false;
  ...
  if (lString.empty() == false)
  {
    ...
    if ((lString == "SENSCODAGE") || (lString == "ORDER"))
    {
      ...
      else
      {
        if (reportError == true)
        {
          itkExceptionMacro(<< "RAD : the value SENSCODAGE '" << lString
...
void RADImageIO::WriteImageInformation()
{
  ...
  {
    itkExceptionMacro(<< "The file " << m_FileName << " is not defined as a RAD file");
  }
  ...
  for (unsigned int i = 0; i < m_NbOfChannels; ++i)
  {
    m_HeaderFile << itksys::SystemTools::GetFilenameName(this->m_ChannelsFileName[i]) << std::endl;
  }
  m_HeaderFile.close();
